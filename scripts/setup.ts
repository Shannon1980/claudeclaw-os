...
async function main() {
  ...
  console.log(`  ClaudeClaw bridges your Claude Code CLI to Slack (or Telegram).`);
  ...
  info('down, the bot goes offline. Messages queue in your chat app and arrive when');
  info('you restart.');
  console.log();
  ...
  const wantWhatsApp = await confirm('WhatsApp bridge? (view and reply to WhatsApp from your chat app, highly experimental)', false);
  ...
  section('Chat app');
  info('ClaudeClaw runs on Slack (primary) or Telegram. Pick your front-end.');
  console.log();
  ...
  const transportDefault = (env.TRANSPORT || '').toLowerCase() === 'telegram' ? '2' : '1';
  const transportChoice = await ask('Use (1) Slack or (2) Telegram?', transportDefault);
  const transport: 'telegram' | 'slack' =
    (transportChoice.trim() === '2' || /telegram/i.test(transportChoice)) ? 'telegram' : 'slack';
  ...
  info('your Slack or Telegram account, they control the bot. These layers protect you.');
  console.log();
  ...
  const lines = [
    ...
    `TRANSPORT=${env.TRANSPORT || 'slack'}`,
    '',
    '# ── Slack front-end (Socket Mode, primary) ────────────────────',
    `SLACK_BOT_TOKEN=${env.SLACK_BOT_TOKEN || ''}`,
    `SLACK_APP_TOKEN=${env.SLACK_APP_TOKEN || ''}`,
    `ALLOWED_SLACK_USER_ID=${env.ALLOWED_SLACK_USER_ID || ''}`,
    '',
    '# ── Telegram front-end (alternative) ──────────────────────────',
  ...
  if (wantWhatsApp) {
    ...
    info('Then use /wa in your chat app to access your chats.');
    console.log();
  ...
  if (transport === 'slack') {
    info('On Slack, agents are delegation-only: you reach them through the main');
    info('bot with "@agent: ..." or /delegate. No extra bots or tokens needed.');
  } else {
  ...
  if (wantAgents) {
    ...
    while (true) {
      ...
      if (fs.existsSync(exampleYaml)) {
        ...
        yamlContent = token
          ? yamlContent.replace(/telegram_bot_token_env:.*/, `telegram_bot_token_env: ${envKey}`)
          // Delegation-only agent (Slack transport): no standalone bot token
          : yamlContent.replace(/^telegram_bot_token_env:.*\n?/m, '');
    ...
    if (createdAgents.length > 0) {
      ...
      console.log();
      if (transport === 'slack') {
        info('Reach them through the main bot on Slack:');
        console.log();
        for (const id of createdAgents) {
          console.log(`  ${c.cyan}@${id}: your request here${c.reset}`);
        }
      } else {
