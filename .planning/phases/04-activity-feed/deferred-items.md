...
| 04-03 (full suite) | `src/chat-task-tracker.test.ts` "returns null when classification unavailable" fails (returns a task id instead of null) | Pre-existing, LLM-environment-dependent failure. Verified failing identically on the commit BEFORE any 04-03 change (parent of the first plan-03 commit). Touches none of this plan's files (undo-executor, approval-queue, dashboard, Activity.tsx). The test asserts the classifier returns null when both the SDK classification and the Gemini fallback are unavailable, but the test environment had a live classifier. Out of scope per the executor scope boundary. |
